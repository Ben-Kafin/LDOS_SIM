...
def tunneling_factor_copilot_iterative(V, E, phi):
    V *= 1.60218e-19
    E *= 1.60218e-19
    prefactor = 8 / (3 * V) * pi * sqrt(2 * 9.11e-31) / 6.626e-34
    
    # Check if phi is a list
    if isinstance(phi, list):
        barrier = []
        for phi_value in phi:
            phi_value *= 1.60218e-19
            barrier.append((phi_value - E + V)**(3 / 2) - (phi_value - E)**(3 / 2))
        return [prefactor * b for b in barrier]
    else:
        phi *= 1.60218e-19
