...
class STM_Simulator:
    ...
    def _find_bulk_surface_z(self, bin_width=0.5125, filter_atom_type=None):
        ...
        # --- NEW: Atom Type Filtering Logic ---
        if filter_atom_type:
            print(f"Filtering for atom type '{filter_atom_type}' for surface detection.")
            # Create a list mapping each atom index to its type
    ...
    def plot_position_slice(self, pos):
        """
        Plots spectra at specific positions (in Angstroms) from the 
        most recent line scan.
        """
        if not hasattr(self, 'scan_data'):
            print("Error: No scan data found. You must run_line_scan() first.")
            return

        # Extract stored data
        energies = self.scan_data['energies']
        distances = self.scan_data['distances']
        spectra = self.scan_data['spectra'] # Shape: (n_points, n_energies)

        # Create plot
        fig, ax = plt.subplots(figsize=(8, 6))
        
        # Handle single float or list input
        if not isinstance(pos, list): 
            pos = [pos]

        # Loop through requested positions and plot closest match
        for p in pos:
            # Find index of the point closest to requested position 'p'
            idx = np.argmin(np.abs(distances - p))
            actual_pos = distances[idx]
            
            # Extract spectrum at that index
            current_spectrum = spectra[idx, :]
            
            ax.plot(energies, current_spectrum, label=f'{actual_pos:.2f} Å')

        ax.set_xlabel("Energy (eV)")
        ax.set_ylabel("LDOS Intensity")
        ax.set_title("Position Slices")
        ax.legend()
        ax.grid(True)
        plt.show()
...
# In LDOS.py, replace the entire run_line_scan method with this one:
    ...
    def run_line_scan(self, path_coords, npts, erange, topo_bias, const_height, locpot_path, surface_mode, bulk_atom_type, cmap_ldos, cmap_topo, normalize_spectra, highlight_positions=None):
            ...
            # 2. Initial Constant Height Scan (Establish Setpoint)
            # We start with a flat line at the requested constant height above the highest atom.
            print(f"\n[STEP 1] Initial Scan to Determine Setpoint (Bias = {topo_bias:.2f}V, Height = {const_height} A)...")
            initial_z = self.z_highest_atom + const_height 
            current_z_profile = np.full(npts, initial_z)
            
            topo_emin, topo_emax = min(0.0, topo_bias), max(0.0, topo_bias)
            
            # Helper to get integrated LDOS at specific Z (used for feedback loop)
            def get_integrated_ldos(z_vals):
                pts = np.hstack([path_coords_xy, z_vals[:, np.newaxis]])
                specs, engs = self._calculate_ldos_at_points(pts, topo_emin, topo_emax)
                # Integrate spectral density to get "current"
                return np.array([simpson(np.sum(s, axis=0), x=engs) for s in specs]), engs
    
            # Calculate initial current at the flat height
            current_ldos_int, _ = get_integrated_ldos(current_z_profile)
            
            # The Setpoint is the current at the "brightest" spot in the flat scan.
            # (This mimics bringing the tip close until the setpoint is reached over the highest atom)
            target_setpoint = np.max(current_ldos_int)
            print(f"Target LDOS Setpoint established: {target_setpoint:.4e} (arb. units)")
    
            # 3. Iterative Feedback Loop (Adjust Z to match Setpoint)
            print("\n[STEP 2] Iteratively adjusting tip height to maintain constant LDOS...")
            tolerance = 0.01  # 1% error tolerance
            max_iter = 30     # Max iterations to prevent infinite loops
            gain = 0.5        # Feedback gain (dampens the correction to prevent oscillation)
            
            for iteration in range(max_iter):
                # Avoid division by zero
                safe_current_ldos = np.maximum(current_ldos_int, 1e-20)
                
                # Ratio > 1 means too bright (move up), Ratio < 1 means too dark (move down)
                ratios = safe_current_ldos / target_setpoint
                
                # Check convergence
                max_error = np.max(np.abs(ratios - 1.0))
                
                # --- ADDED STATS CALCULATION HERE ---
                z_min = np.min(current_z_profile)
                z_max = np.max(current_z_profile)
                z_range = z_max - z_min
                mean_z = np.mean(current_z_profile)

                print(f"   Iter {iteration+1}: Max Error = {max_error*100:.2f}%, Mean Z = {mean_z:.2f} A, Z Range = {z_range:.4f} A (Min: {z_min:.4f}, Max: {z_max:.4f})")
                
                if max_error < tolerance:
                    print("   Convergence reached.")
                    break
                
                # Update Z positions: z_new = z_old + gain * ln(current / setpoint)
                # This relies on I ~ exp(-2*kappa*z) -> ln(I) ~ -2*kappa*z
                z_correction = gain * np.log(ratios)
                current_z_profile += z_correction
                
                # Recalculate LDOS at new heights for next iteration
                current_ldos_int, _ = get_integrated_ldos(current_z_profile)
    
            variable_z = current_z_profile
    
            # 4. Final Spectroscopic Scan
            print(f"\n[STEP 3] Final Spectroscopic Scan at Converged Coordinates...")
            ...
            self.scan_data = {
                'spectra': ldos_plot_data,
                'energies': final_energies,
                'distances': path_distance
            }
    
            fig = plt.figure(figsize=(10, 9))
            gs = fig.add_gridspec(2, 2, width_ratios=[0.2, 3], height_ratios=[2.5, 1], wspace=0.05, hspace=0.3)
            ax_topo_stripe = fig.add_subplot(gs[0, 0])
            ax_ldos = fig.add_subplot(gs[0, 1], sharey=ax_topo_stripe)
            ax_topo_line = fig.add_subplot(gs[1, :])
            
            energies_mesh, distance_mesh = np.meshgrid(final_energies, path_distance)
            
            ldos_map = ax_ldos.pcolormesh(
                energies_mesh, distance_mesh, ldos_plot_data, 
                cmap=cmap_ldos, shading='gouraud', rasterized=True 
            )
            
            # Draw dashed lines for slice positions if requested
            if highlight_positions:
                for pos in highlight_positions:
                    ax_ldos.axhline(y=pos, color='white', linestyle='--', linewidth=1.5, alpha=0.9)
    
            fig.colorbar(ldos_map, ax=ax_ldos, label='Normalized LDOS' if normalize_spectra else 'LDOS (arb. units)')
            ax_ldos.set(xlabel="Energy (eV)"); ax_ldos.yaxis.set_ticks_position('right')
            
            # Draw the Topography Stripe (Color bar on the left)
            ...
            cax = fig.add_axes([0.05, 0.45, 0.02, 0.45])
            fig.colorbar(lc, cax=cax, label='Tip Z-Height (Å)'); cax.yaxis.set_ticks_position('left'); cax.yaxis.set_label_position('left')
            
            # Draw the Topography Line Plot (Bottom)
            ax_topo_line.plot(path_distance, variable_z)
            ax_topo_line.set(ylabel=r'Tip Z-Height ($\AA$)', xlabel=r'Distance Along Path ($\AA$)', 
                             title='Converged Constant-Current Topography', ylim=(0, self.lv[2, 2]))
            ax_topo_line.grid(True)
            
            title_str = (f"Path: ({path_coords[0]:.2f}, {path_coords[1]:.2f}) -> ({path_coords[2]:.2f}, {path_coords[3]:.2f}) | "
                         f"Setpoint Defined at Height: {const_height:.1f} Å\n"
                         f"Topo Bias: {topo_bias:.2f} V | Spec Range: {erange}")
            fig.suptitle(title_str, y=0.98)
            plt.show()

    def run_map_scan(self, npts_2d, erange, num_energy_points, topo_bias, const_height, locpot_path, plot_cells, surface_mode, bulk_atom_type, cmap_ldos, cmap_topo, normalize_spectra):
            print("--- Running Constant-Current (Iterative) 2D Map Scan ---")
            self.parse_vasp_outputs(locpot_path, surface_mode, bulk_atom_type)
            ...
            print(f"\n[STEP 1] Initial Scan to Determine Setpoint (Bias = {topo_bias:.2f}V, Height = {const_height} A)...")
            initial_z = self.z_highest_atom + const_height
            current_z_profile = np.full(nx * ny, initial_z)
            
            topo_emin, topo_emax = min(0.0, topo_bias), max(0.0, topo_bias)
            
            # Helper for feedback loop
            def get_integrated_ldos(z_vals):
                pts = np.hstack([map_coords_xy, z_vals[:, np.newaxis]])
                specs, engs = self._calculate_ldos_at_points(pts, topo_emin, topo_emax)
                return np.array([simpson(np.sum(s, axis=0), x=engs) for s in specs])
    
            current_ldos_int = get_integrated_ldos(current_z_profile)
            target_setpoint = np.max(current_ldos_int)
            print(f"Target LDOS Setpoint established: {target_setpoint:.4e} (arb. units)")
            
            # 3. Iterative Feedback Loop
            print("\n[STEP 2] Iteratively adjusting tip height...")
            tolerance = 0.01; max_iter = 20; gain = 0.5
            ...
            for iteration in range(max_iter):
                ...
                print(f"   Iter {iteration+1}: Max Error = {max_error*100:.2f}%, Mean Z = {np.mean(current_z_profile):.2f} A, Z Range = {z_range:.4f} A (Min: {z_min:.4f}, Max: {z_max:.4f})")
                ...
                current_z_profile += gain * np.log(ratios)
                current_ldos_int = get_integrated_ldos(current_z_profile)
            ...
            final_spectra_list, final_energies = self._calculate_ldos_at_points(
                tip_positions_variable, erange[0], erange[1], target_energies=target_energies
            )
            ...
            topography_map = variable_z.reshape((ny, nx))
            
            # 5. Plotting
            title_str = (f"Setpoint Height: {const_height:.1f} Å | Topo Bias: {topo_bias:.2f} V")
    
            # Topography Plot
            fig_topo, ax_topo = plt.subplots(figsize=(10, 10))
            topo_plot_data = np.tile(topography_map, (2*plot_cells+1, 2*plot_cells+1))
            
            # Reconstruct extended grid for plotting
            all_x, all_y = [], []
            for i in range(-plot_cells, plot_cells + 1):
                for j in range(-plot_cells, plot_cells + 1):
                    offset = i * self.lv[0, :2] + j * self.lv[1, :2]
                    all_x.append(map_coords_xy[:,0] + offset[0])
                    all_y.append(map_coords_xy[:,1] + offset[1])
                    
            contour = ax_topo.tricontourf(np.concatenate(all_x), np.concatenate(all_y), topo_plot_data.ravel(), levels=100, cmap=cmap_topo)
            fig_topo.colorbar(contour, ax=ax_topo, label='Tip Height (Å)')
            ax_topo.set_aspect('equal')
            ax_topo.set(xlabel='X ($\AA$)', ylabel='Y ($\AA$)', title=f"Topography Map\n{title_str}")
            plt.show()
            ...
            for e_idx, energy in enumerate(final_energies):
                ...
                filename = join(output_dir, f"ldos_map_E_{energy:.3f}eV.png")
...
if __name__ == "__main__":
    ...
    bulk_atom_type = 'Au' # Set to atom symbol (e.g., 'Au') or None to use all atoms
    normalize_final_spectra = True
    
    cmap_ldos = 'jet'
    from matplotlib.colors import LinearSegmentedColormap
    # Define the custom map: Black (low) -> Firebrick (mid) -> Yellow (high)
    custom_topo = LinearSegmentedColormap.from_list("custom_topo", ["black", "firebrick", "yellow"])
    cmap_topo = custom_topo 
    # ----------------------
    ...
    if simulation_mode == 'point':
        ...
        scan_path_coords = [4.00869,  23.77771, 13.27028,  -8.77865]
        number_of_points = 72
        energy_range = [-2.54, -1.34]
        topo_bias_voltage = -2.54
...
if __name__ == "__main__":
    ...
    simulation_mode = 'line'
    vasp_directory = r'C:/Users/Benjamin Kafin/Documents/VASP/SAM/zigzag/kpoints551/dpl_corr/kp551'
    locpot_file_path = "LOCPOT"
    surface_detection_mode = 'bulk'
    bulk_atom_type = 'Au' 
    normalize_final_spectra = True
    ...
    if simulation_mode == 'line':
        scan_path_coords = [13.27028,  -8.77865, 4.00869,  23.77771]
        #scan_path_coords = [34.87265,  30.65221,  -6.56552,  -7.22864]
        number_of_points = 72
        energy_range = [-2.525, -1.3]
        topo_bias_voltage = -2.525
        const_scan_height = 1.125
        #base_tip_distance = 6.4
        #tip_height_scaler = 10
        
        # --- DEFINE SLICE POSITIONS HERE (in Angstroms along the path) ---
        my_slices = [17.75, 24.8] 
        # -----------------------------------------------------------------
    ...
    if simulation_mode == 'line':
        sim.run_line_scan(
            path_coords=scan_path_coords, 
            npts=number_of_points, 
            erange=energy_range, 
            topo_bias=topo_bias_voltage, 
            const_height=const_scan_height, 
            #scaler=tip_height_scaler, 
            #base_dist=base_tip_distance,
            locpot_path=locpot_file_path, 
            surface_mode=surface_detection_mode,
            bulk_atom_type=bulk_atom_type,
            cmap_ldos=cmap_ldos, 
            cmap_topo=cmap_topo, 
            normalize_spectra=normalize_final_spectra,
            highlight_positions=my_slices  # <--- Pass positions here for dashed lines
        )
        
        # --- Create the separate plot with curves ---
        sim.plot_position_slice(pos=my_slices)