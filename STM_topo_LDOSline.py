# -*- coding: utf-8 -*-
"""
Created on Sun Mar  1 13:41:11 2026
...
class Interactive_STM_Simulator(Unified_STM_Simulator):
    def __init__(self, filepath, erange, ldos_height, cmap_topo):
        ...
        # Default path: Diagonal of the unit cell
        self.p1 = np.array([0.0, 0.0])
        self.p2 = self.lv[0, :2] + self.lv[1, :2]
        ...
        # High-contrast colors for markers and spectra (avoids red/yellow)
    ...
    def run_interactive(self, grid_res=64, topo_bias=0.2, topo_height=2.5, ldos_bias_sign='neg', use_decay_topo=True, use_decay_ldos=True):
        ...
        for s in [self.s_cell, self.s_emin, self.s_emax, self.s_num_marks]: s.on_changed(self._on_ui_change)
    ...
    def _update_all(self, full_refresh=False):
        ...
        if self.show_mag and f_dn is not None:
            v_max = np.max(np.abs(f_ldos)); mesh = self.ax_ldos.pcolormesh(self.cached_eg, p_dist, f_ldos, cmap='bwr', shading='auto', vmin=-v_max, vmax=v_max)
        else:
        ...
        for i, r in enumerate(self.marker_ratios):
            ...
            self.ax_spec.plot(self.cached_eg, f_ldos[idx], color=color, label=f'{p_dist[idx]:.2f} Å')
        ...
        self.ax_spec.set(title="Single Point LDOS", xlabel="Energy (eV)"); self.ax_spec.legend(fontsize='x-small')
    ...
    def _on_motion(self, event):
        ...
        elif t_obj == 'mark_dynamic':
            # This handles Map, Profile (xdata), and Heatmap (ydata)
            if event.inaxes == self.ax_prof:
                if p_len > 1e-9: self.marker_ratios[idx] = np.clip(event.xdata / p_len, 0, 1)
            elif event.inaxes in [self.ax_ldos, self.ax_stripe]:
    ...
    def _on_ui_change(self, val):
        ...
        if new_count != len(self.marker_ratios):
            if new_count > len(self.marker_ratios): # Add equidistant points
...
if __name__ == "__main__":
    ...
    sim = Interactive_STM_Simulator(v_dir, [-2.525, -1.3], 1.25, LinearSegmentedColormap.from_list("t", ["black", "firebrick", "yellow"]))
