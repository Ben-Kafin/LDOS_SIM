...
class STM_Simulator:
    ...
    def run_line_scan(self, path_coords, npts, erange, topo_bias, const_height, scaler, base_dist, locpot_path, surface_mode, bulk_atom_type, cmap_ldos, cmap_topo, normalize_spectra):
        ...
        ldos_map = ax_ldos.pcolormesh(energies_mesh, distance_mesh, ldos_plot_data, cmap=cmap_ldos, shading='gouraud')
        ...
        ax_topo_line.set(ylabel=r'Tip Z-Height ($\AA$)', xlabel=r'Distance Along Path ($\AA$)', 
                         title='Calculated Tip Topography', ylim=(0, self.lv[2, 2]))
    ...
    def run_map_scan(self, npts_2d, erange, num_energy_points, topo_bias, const_height, scaler, base_dist, locpot_path, plot_cells, surface_mode, bulk_atom_type, cmap_ldos, cmap_topo, normalize_spectra):
        ...
        ax_topo_map.set(xlabel='X ($\AA$)', ylabel='Y ($\AA$)'); ax_topo_map.set_title(title_str)
        plt.show()
...
if __name__ == "__main__":
    ...
    if simulation_mode == 'point':
        ...
        number_of_points = 64
        energy_range = [-2.575, -1.8]
        topo_bias_voltage = -0.2
        const_scan_height = 5.0
        base_tip_distance = 2.0
        tip_height_scaler = 11.5
